...
#distribution of label by country data frame
...
# adding 2 palettes together to get color fills for all languages by country for plot 2
my_palette = c(brewer.pal(9, "Pastel1"), brewer.pal(8, "YlGn"))


#distribution of tweets by language by country data frame
df2 = afrosenti%>%select(country,language,tweet)%>%group_by(language, country)%>%count()%>%ungroup()%>%
  group_by(country)%>%mutate(percent = round(n/sum(n)*100.0,2),xpos = cumsum(percent))%>%arrange(country)


#2nd plot

p2 = ggplot(df2, aes(x = percent, y = reorder(country, desc(country)),fill = factor(language)))+geom_col(colour = "black")+
